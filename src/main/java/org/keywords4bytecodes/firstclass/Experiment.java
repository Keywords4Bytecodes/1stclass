...
public class Experiment {
		...
		for (BytecodeData clearClass : clearData) {
			...
			if (obfuscated != null) {
				...
				if (obMethods.size() != clearClass.getMethods().size())
					System.err.println(
							"loadObfuscatedFiles: skipping " + clearClass.getClassName() + " length mismatch, clear: "
									+ clearClass.getMethods().size() + " obfuscated: " + obMethods.size());
	...
	public Results crossValidate(int folds, Random random) {
		...
		try {
			...
			Collections.shuffle(shuffled, random); // shuffle train data in
													// place
	...
	public static void main(String[] args) throws Exception {
		...
		if (args[0].equals("train") || args[0].equals("fulltrain") || args[0].equals("baseline")
			...
			Experiment exp = new Experiment(system, vocab);
			// get aligned jars on folder, obfuscated_ and original_
			Map<String, File> files = new HashMap<>();
			for (File f : new File(args[1]).listFiles())
				files.put(f.getName(), f);
			for (String fname : files.keySet())
				if (fname.endsWith(".jar") && fname.startsWith("original_")) {
					String obfname = fname.replace("original_", "obfuscated_");
					File obfuscated = files.get(obfname);
					if (obfuscated != null)
						exp.addObfuscatedData(files.get(fname), obfuscated, DataType.TEST);
				}
			System.out.println("Testing data size: " + exp.getTestData().size());
